...
class Printbarcode extends CI_Controller {
    ...
    public function press()
    {
        if(!empty($this->input->get('id'))){
            $id = $this->input->get('id');
            if(isset($id)){
                $barcode_detail = $this->printbarcode_model->get_barcode_stock_press_by_id($id);
                ...
                if($barcode_detail){
                    ...
                    foreach($barcode_detail as $each_barcode){
                        $temp_name = substr($each_barcode['bahan_dasar'], 0, 16);
                        $description_line1 = ""; // max 16 character
                        if($temp_name){
                            $description_line1 = $temp_name;
                        }

                        $temp_name = substr($each_barcode['bahan_dasar'], 16, 16);
                        $description_line2 = ""; // max 16 character
                        if($temp_name){
                            $description_line2 = $temp_name;
                        }

                        $code_to_encode = $each_barcode['stock_press_code'];; // max 7 character
                        $quantity = $each_barcode['label_quantity'];;
