(function($) {
	$(document).ready(function(e) {
        $("table#da-kirimpress-barcode-print-datatable-numberpaging").DataTable({
        ...
        $('#da-kirimpress-barcode-print-confirmation-submit').on('click', function(event) {
            event.preventDefault();
        });

        $('#da-kirimpress-barcode-print-submit').on('click', function(event) {
            event.preventDefault();
            ...
            // get input detail
            $("#da-kirimpress-barcode-print-detail-error").hide();
            $('table#da-kirimpress-barcode-print-datatable-numberpaging tr').each(function(outer_index) {
                if(outer_index > 0){
            ...
            if(walk > 0){
                if(overflowStatus == false){
                    ...
                    $('#da-kirimpress-barcode-print-submit-item-values').val(barcodePrintValues);

                    // submit form
                    $('form#da-kirimpress-barcode-print-detail-form-val').submit();
                }else{
                    var message = "Jumlah label lebih besar dari barang diterima.";
                    $("#da-kirimpress-barcode-print-detail-error").html(message).show();
                }
            }else{
