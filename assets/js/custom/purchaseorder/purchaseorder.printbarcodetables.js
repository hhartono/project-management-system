(function($) {
	$(document).ready(function(e) {
        $("table#da-purchaseorder-barcode-print-datatable-numberpaging").DataTable({
        ...
        $('#da-purchaseorder-barcode-print-submit').on('click', function(event) {
            event.preventDefault();
            ...
            // get input detail
            $("#da-purchaseorder-barcode-print-detail-error").hide();
            $('table#da-purchaseorder-barcode-print-datatable-numberpaging tr').each(function(outer_index) {
                if(outer_index > 0){
            ...
            if(walk > 0){
                if(overflowStatus == false){
                    ...
                    $('#da-purchaseorder-barcode-print-submit-item-values').val(barcodePrintValues);

                    // submit form
                    $('form#da-purchaseorder-barcode-print-detail-form-val').submit();
                }else{
                    var message = "Jumlah label lebih besar dari barang diterima.";
                    $("#da-purchaseorder-barcode-print-detail-error").html(message).show();
                }
            }else{
